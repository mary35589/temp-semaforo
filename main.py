...
sensor_temperatura=ADC(Pin(35))
sensor_temperatura.atten(ADC.ATTN_11DB)
...
def temperatura(Pin): 
   ...
   if muestra_temp:
      muestra_temp=False
      print("No se va a tomar temperatura")

   else:
      muestra_temp=True
      print("Se va a tomar temperatura")

pul_interrupcion=Pin(34,Pin.IN,Pin.PULL_DOWN)
pul_interrupcion.irq(trigger=Pin.IRQ_RISING, handler=interrupcion)

pul_sensor=Pin(32,Pin.IN, Pin.PULL_DOWN)
pul_sensor.irq(trigger=Pin.IRQ_RISING, handler=temperatura)

while True:
   if muestra_temp:
      toma= sensor_temperatura.read()
      temperatura = toma*conversion #para convertir la muestra a grados celsius
